...
registerLicense(
  "ORg4AjUWIQA/Gnt2VVhjQlFaclhJXGFWfVJpTGpQdk5xdV9DaVZUTWY/P1ZhSXxRd0djXH5ccn1QTmFeUUA="
);
...
export default function App({ Component, pageProps }: AppProps) {
  ...
  const [sidenavExpanded, setSidenavExpanded] = useState(false); // Usestate is not used correctly 
  return (
    <div className="flex w-full h-full">
      <Sidenav />
      <main className="bg-gray-200 h-full w-full absolute"
      style={{ left: sidenavExpanded ? '10rem' : '4rem' }}>
        <Component {...pageProps} />
      </main>
    </div>
  );
